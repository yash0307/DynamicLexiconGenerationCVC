...
#    f = open("results.txt", "w")
    ...
    if ((os.path.isfile("./jad_gensim_dict.txt")) and (os.path.isfile("/media/DADES/yash/Wiki/wiki_en_bow.mm"))):
	    pass
    else :
    ...
    id2word = gensim.corpora.Dictionary.load_from_text('./jad_gensim_dict.txt')
    ...
    if ((analysis_type.lower() == "lda") and not (os.path.isfile("./lda_model.lda"))):
	    ...
	    lda_model = gensim.models.ldamodel.LdaModel.load('./lda_model.lda', mmap='r')

    if ((analysis_type.lower() == "lsa") and not (os.path.isfile("./lsa_model.lsi"))):
	    lsa_model = gensim.models.lsimodel.LsiModel(corpus = mm, id2word=id2word ,num_topics = 400)
	    lsa_model.save('./lsa_model.lsi')
	    lsa_model.print_topics(10)
    elif (analysis_type.lower() == "lsa") :
	    lsa_model = gensim.models.LsiModel.load('./lsa_model.lsi')
